...
public class UnaryArenaServiceImpl implements UnaryArenaService {
    ...
    // Rounds per match. Defaults to 1000 (the "painful polling" showcase); tests
    // and CI override it (e.g. arena.total-rounds=20) for a fast full match.
    ...
    public Uni<SubmitMoveResponse> submitMove(SubmitMoveRequest request) {
        ...
        return UnaryMatch.findByMatchIdForUpdate(request.getMatchId())
            .chain(match -> {
                if (match == null) {
